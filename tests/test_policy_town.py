...
class TownAndFundraisingPolicyTest(shop_fixture._TownShopFixtureBase):
    ...
    def test_detected_mining_requires_food_and_digger_until_suppliers_fail(self):
        ...
        snap = Snapshot(
            ...
            floor_key=(0, 0, 0), town_flag=True,
            inventory=[
                it for it in self._strict_supplies(detection=1)
...
class OrganizationDepartureRecordedTest(unittest.TestCase):
    """Pin the optional organization errand on the terminal incident board."""
    ...
    def setUpClass(cls):
        ...
        cls.fixture = (
            Path(__file__).parent
            / "fixtures"
            / "organization-blocks-departure.jsonl.gz"
        )
        assert hashlib.sha256(cls.fixture.read_bytes()).hexdigest() == (
            "8f9b623bdb61a8b2c7c9f1d96b7752752bb02143de30fd3a29988299be3b0d32"
        )
        with gzip.open(cls.fixture, "rt", encoding="utf-8") as stream:
            cls.home, cls.terminal = [
                parse_snapshot(json.loads(line), cls.monrace) for line in stream
            ]

    def test_o1_nonfull_surplus_reaches_the_departure_path(self):
        policy = HengbotPolicy(monrace_knowledge=self.monrace)
        self.assertTrue(policy.consume_home_knowledge(self.home.store.items))
        surplus = policy._find_town_organization_surplus(self.terminal)

        self.assertIsNotNone(surplus)
        self.assertEqual(len(self.terminal.inventory), 15)
        # Equipment search state is absent from the JSON-view capture. Wall
        # that unrelated collaborator only after the real surplus producer.
        with patch.object(policy, "_equipment_departure_ready", return_value=True):
            self.assertTrue(policy._town_departure_ready(self.terminal))
            self.assertFalse(policy._descent_is_blocked(self.terminal))

    def test_o2_full_recorded_pack_still_fails_organization(self):
        fixture = (
            Path(__file__).parent
            / "fixtures"
        ...
        with gzip.open(fixture, "rt", encoding="utf-8") as stream:
            full = parse_snapshot(json.loads(next(stream)), {})
        policy = HengbotPolicy(monrace_knowledge=self.monrace)

        self.assertTrue(
            policy._town_departure_conjuncts(self.terminal)["organization_complete"]
        )
        self.assertEqual(len(full.inventory), PACK_CAPACITY)
        self.assertIsNotNone(policy._find_town_organization_surplus(full))
        self.assertFalse(policy._town_departure_conjuncts(full)["organization_complete"])

    def test_o3_nonfull_organization_errand_keeps_its_priority(self):
        policy = HengbotPolicy(monrace_knowledge=self.monrace)
        surplus = policy._find_town_organization_surplus(self.terminal)
        ...
        organization = [
            need
            for need in policy._enumerate_town_needs(self.terminal)
            if need.category == "organization-sale"
        ]
