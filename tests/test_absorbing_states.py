...
class AbsorbingStateHarnessTest(unittest.TestCase):
    def test_catalogue_is_cheap_and_grows_by_data(self):
        ...
        self.assertEqual(len(SEEDED_STATES), 16)
        self.assertEqual(len({state.name for state in SEEDED_STATES}), 16)
        self.assertEqual(len({state.build for state in SEEDED_STATES}), 16)
