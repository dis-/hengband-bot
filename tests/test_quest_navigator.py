...
class QuestFloorNavigatorTest(unittest.TestCase):
    ...
    def snapshot(self, y, x, *, status=QUEST_STATUS_TAKEN, monsters=(), loot=None, open_doors=False):
        return Snapshot(
            player(y, x, hp=100, max_hp=100),
            grids_for(self.battlefield, open_doors=open_doors, loot=loot),
    ...
    def test_sweep_collects_then_exit_searches_sealed_door_and_ascends(self):
        loot = self.snapshot(8, 6, status=QUEST_STATUS_COMPLETED, loot=(8, 6))
        self.assertEqual(self.policy.choose_key(loot), ",")
        self.assertEqual(self.policy.last_reason, "quest:sweep:pickup")
        at_door = self.snapshot(8, 5, status=QUEST_STATUS_COMPLETED)
        self.assertEqual(self.policy.choose_key(at_door), "s")
        self.assertEqual(self.policy.last_reason, "quest:exit:search-door")
        opened = self.snapshot(8, 5, status=QUEST_STATUS_COMPLETED, open_doors=True)
        self.assertEqual(self.policy.choose_key(opened), "4")
        at_exit = self.snapshot(8, 1, status=QUEST_STATUS_COMPLETED, open_doors=True)
    ...
    def test_sealed_door_search_is_bounded(self):
        snap = self.snapshot(8, 5, status=QUEST_STATUS_COMPLETED)
        reasons = []
    ...
    def test_full_floor_episode_never_emits_a_generic_reason(self):
        episode = [
            ...
            self.snapshot(8, 2),
            self.snapshot(8, 3),
            self.snapshot(8, 3, monsters=[hostile(1, 8, 4)]),
            self.snapshot(8, 5, status=QUEST_STATUS_COMPLETED),
            self.snapshot(8, 5, status=QUEST_STATUS_COMPLETED, open_doors=True),
            self.snapshot(8, 4, status=QUEST_STATUS_COMPLETED, open_doors=True),
            self.snapshot(8, 3, status=QUEST_STATUS_COMPLETED, open_doors=True),
            self.snapshot(8, 2, status=QUEST_STATUS_COMPLETED, open_doors=True),
            self.snapshot(8, 1, status=QUEST_STATUS_COMPLETED, open_doors=True),
        ]
