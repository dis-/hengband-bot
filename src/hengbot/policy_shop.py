...
class ShopMixin:
    ...
    def _store_item_stacks_with_inventory(pack: InventoryItem, item: StoreItem) -> bool:
        # Ordinary-shop JSON does not export base dice or known flags (unlike
        # inventory/Home rows).  Treating those omitted fields as literal
        # zero/empty values rejects the exact plain ammunition which the game
        # will merge into an existing plain stack.  The visible store identity
