...
class ShopMixin:
    ...
    @staticmethod
    def _purchase_item_identity(item) -> tuple[object, ...]:
        return (
            item.name, item.tval, item.sval, getattr(item, "charges", 0),
            getattr(item, "aware", True),
        )

    def _purchase_rungs(self, context: PurchaseContext) -> tuple[PurchaseRung, ...]:
        """Return the ordered, live authority shared by buying and retention."""
        snapshot = context.snapshot
        ledger = self._supply_ledger(snapshot, self._planned_depth())
        strategy = self._carry_procurement_strategy(snapshot)

        def rung(rung_id, category, active, compatible, *, current=None, target=None, rationale=""):
            def match(ctx, item):
                if not active() or not compatible(item):
                    return None
                cur = current() if current is not None else None
                goal = target() if target is not None else None
                shortage = None if cur is None or goal is None else max(0, goal - cur)
                return PurchaseMatch(
                    rung_id, category, self._purchase_item_identity(item),
                    cur, goal, shortage, rationale or rung_id,
                )
            return PurchaseRung(rung_id, category, match)
