...
class HomeMixin:
    ...
    def _bind_catalogued_home_identification_withdrawal(
        ...
        if (
            ...
            item = next(
                ...
                for index, item in enumerate(self._home_knowledge_items)
                if index < self._home_knowledge_valid_before
                and self._item_signature(item) == self._identification_candidate
            )
            full = bool(
                item.known
                and item_requires_full_identification(item)
    ...
    def _home_identification_candidate_pending(
        ...
        known_source = any(
            item.tval == TVAL_SCROLL
            and item.sval in {SV_SCROLL_IDENTIFY, SV_SCROLL_STAR_IDENTIFY}
